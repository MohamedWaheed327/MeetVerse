...
public class DeepGramController : ControllerBase
{
    ...
    static string? SimplifyDeepgramMessage(string rawJson)
    {
        try
        {
            ...
            if (!channelProp.TryGetProperty("alternatives", out var alternativesProp))
                return null;

            if (alternativesProp.GetArrayLength() == 0)
                return null;
        ...
        var browserToDeepgram = Task.Run(async () =>
        {
            try
            {
                while (browserSocket.State == WebSocketState.Open &&
                       deepgramSocket.State == WebSocketState.Open &&
                       !cts.Token.IsCancellationRequested)
                {
                    var message = await ReceiveMessageAsync(browserSocket, cts.Token); // recieve audio from browser

                    if (message.MessageType == WebSocketMessageType.Close) // no audio, browser is closed
                    {
                        if (deepgramSocket.State == WebSocketState.Open) // tell deepgram to close, as browser is closed
                        {
                            await deepgramSocket.CloseAsync(
                    ...
                    {
                        await deepgramSocket.SendAsync(
