...
public class GroupChatHub : Hub
{
    ...
    public GroupChatHub(MeetVerseDbContext db) { _db = db; }
    ...
    public async Task JoinGroup(Guid groupId)
    {
        ...
        if (!isMember) { await Clients.Caller.SendAsync("Error", "Not a member of this group"); return; }
        await Groups.AddToGroupAsync(Context.ConnectionId, GroupPrefix + groupId);
        await Clients.Caller.SendAsync("JoinedGroup", groupId);
    }

    public async Task LeaveGroup(Guid groupId) => await Groups.RemoveFromGroupAsync(Context.ConnectionId, GroupPrefix + groupId);
    ...
    public async Task SendMessage(Guid groupId, string content)
    {
        ...
        var msg = new GroupMessage { Id = Guid.NewGuid(), GroupId = groupId, SenderId = userId.Value, Content = content, SentAt = DateTime.UtcNow };
        _db.GroupMessages.Add(msg);
        await _db.SaveChangesAsync();
        var payload = new { msg.Id, msg.GroupId, SenderId = msg.SenderId, SenderName = sender?.Name, SenderAvatarUrl = sender?.AvatarUrl, msg.Content, msg.SentAt, msg.EditedAt };
