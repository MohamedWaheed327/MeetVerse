...
public class WhiteboardHub : Hub
{
    ...
    public WhiteboardHub(MeetVerseDbContext db) { _db = db; }
    ...
    public async Task JoinSession(Guid sessionId)
    {
        ...
        if (!await CanAccessSession(sessionId, userId.Value)) { await Clients.Caller.SendAsync("Error", "Session not found or access denied."); return; }
    ...
    public async Task SendWhiteboardEvent(Guid sessionId, string type, string payloadJson)
    {
        ...
        var evt = new WhiteboardEvent { Id = Guid.NewGuid(), SessionId = sessionId, UserId = userId.Value, Type = type, PayloadJson = payloadJson, CreatedAt = DateTime.UtcNow };
        _db.WhiteboardEvents.Add(evt);
        await _db.SaveChangesAsync();
        var user = await _db.Users.FindAsync(userId.Value);
        var payload = new { evt.Id, evt.SessionId, UserId = evt.UserId, UserName = user?.Name, evt.Type, evt.PayloadJson, evt.CreatedAt };
