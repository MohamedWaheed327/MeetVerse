...
export const getActiveMeetings = async () => {
    try {
        ...
        const processedData = response.data
            .filter((meeting: { title: string }) => meeting.title !== "Instant Meeting")
            .map((meeting: { scheduledStart: string, scheduledEnd: string, id: string, title: string, description: string }) => ({
                ...meeting,
